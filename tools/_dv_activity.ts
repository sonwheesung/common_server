...
// ── ⑦ 계측 경계 판정 (2026-09-02) ───────────────────────────────────────────
// 앱이 AppState 리스너를 붙이는 날 DAU 가 뛴다 — 사용자가 는 게 아니라 세는 방법이 바뀐 것이다.
// 화면이 그 경계를 그릴지 말지가 이 한 함수에 걸려 있고, **DB 를 통해서는 참인 경우를 못 밟는다**
// (지금 네 앱 전부 warmSince=null 이라 대조군이 한쪽뿐이다). 그래서 여기서 네 조합을 다 밟는다.
{
  // ① 아직 안 붙임 — 경계도 없다. null 인데 true 면 화면이 **없는 경계선을 그린다.**
  check('경계: 웜을 못 셌으면 경계 없음', warmBoundaryOf('2026-08-01', null) === false);

  // ② 첫날부터 붙어 있었다 — 경계 없음. 여기가 이 함수의 존재 이유다.
  //    `warmSince !== null` 로만 판정하면 이 경우에 경고가 붙어 **정상을 이상으로** 말하게 된다.
  //    (idea_repository 가 그 경우다 — 프로덕션 첫 빌드에 하트비트가 들어 있다.)
  check('경계: 첫날부터 웜이면 경계 없음', warmBoundaryOf('2026-09-05', '2026-09-05') === false);

  // ③ 나중에 붙었다 — 경계 있음. 이게 my_word·linkmemo·jogak 이 겪을 상태다.
  check('경계: 나중에 붙었으면 경계 있음', warmBoundaryOf('2026-08-01', '2026-09-05') === true);

  // ④ 활성 기록 자체가 없다 — 판정할 것이 없다(0으로 나누지 않는다).
  check('경계: 활성 기록이 없으면 경계 없음', warmBoundaryOf(null, '2026-09-05') === false);
  check('경계: 둘 다 없으면 경계 없음', warmBoundaryOf(null, null) === false);

  // ⚠ 문자열 비교가 날짜 비교와 같은지 — 'YYYY-MM-DD' 는 사전순 = 시간순이라 성립하지만,
  //   자리수가 흔들리면(예: '2026-9-5') 그 성질이 깨진다. 월/일 경계를 넘겨 확인한다.
  check('경계: 월을 넘어도 순서가 맞다', warmBoundaryOf('2026-08-31', '2026-09-01') === true);
  check('경계: 연을 넘어도 순서가 맞다', warmBoundaryOf('2026-12-31', '2027-01-01') === true);
  check('경계: 웜이 더 이르면(있을 수 없는 값) 경계 없음', warmBoundaryOf('2026-09-05', '2026-08-01') === false);
}
...
const MIN_CHECKS = 47; // 2026-09-02: 계측 경계 8건 추가
{
  const ran = pass + fail;
  if (ran < MIN_CHECKS) {
    fail++;
