...
if (TOKEN) {
  ...
  if (codes.length === 0) {
    ...
    // 🔴 대조군이 둘 다 있어야 이 검사에 의미가 있다 — "전부 단일 종류"인 상태에서는
    // **앱 코드를 하드코딩한 판정도 이 가드를 통과한다.**
    //
    // ⚠ 처음엔 이걸 NOTE(로그만)로 뒀다가 고쳤다(2026-09-02). NOTE 는 아무것도 막지 않으면서
    //   "봤다"는 인상만 준다 — 오늘 세 번 나온 "안 도는데 초록" 의 또 다른 얼굴이다.
    //   그리고 else 가지에 `check(..., true)` 를 쓰고 있었는데, 그건 **절대 실패할 수 없으면서
    //   개수만 +1** 하는 허위 오라클이라 바닥(MIN_CHECKS)까지 함께 속인다.
    //   조건 자체를 단언으로 바꾸면 둘 다 사라진다.
    // → 대조군이 사라지면 FAIL 이다. 정말로 모든 앱이 단일 구조가 되는 날이 오면 그건
    //   사람이 봐야 할 변화이지, 가드가 조용히 넘어갈 일이 아니다.
    check(
      '2행/단일 대조군이 둘 다 있다 (없으면 하드코딩 여부를 판별할 수 없다)',
      dualSeen > 0 && singleSeen > 0,
      `dual=${dualSeen} single=${singleSeen}`,
    );
  }
}
