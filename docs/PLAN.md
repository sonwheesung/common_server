...
**관리자 토큰 보관(2026-09-01 정정)**: sessionStorage → localStorage + **30일 슬라이딩 만료**.
종전에는 탭을 닫을 때마다 토큰을 다시 붙여넣어야 했다. 배구 콘솔은 localStorage에 무기한으로
두는데, 여긴 거기에 만료를 더했다 — 쓰는 동안엔 안 끊기고(열 때마다 연장) 방치한 기기에선 결국 만료된다.
저장은 여전히 **서버에서 통한 뒤에만** 하고, 401은 한 곳(`api()`)에서 토큰을 버린다.
⚠ 대가를 숨기지 않는다 — 토큰이 디스크에 남아 브라우저 재시작을 견딘다. 이 화면에 HTML 주입
싱크가 없어서(dangerouslySetInnerHTML 없음) 허용한 것이고, 로그인 카드가 "공용 PC라면 로그아웃"을 명시한다.
...
**정정(2026-09-01 같은 날)** — 되돌렸다. 그렇게 묶으면 `lastSeenAt`이 **그날 첫 접속 시각**에 고정돼
하루 안의 해상도가 사라진다. "지금 몇 명 있나"(최근 30분)를 그 순간 만들 수 없게 되고, 시간대 축도 잃는다.
단일 행 PK UPDATE라 **아낀 값이 잃은 것보다 작았다** — 지금은 하트비트마다 갱신한다.
두 사실의 역할이 다르다: `subject_active_day`는 **덮어써지지 않는 과거**, `lastSeenAt`은 **지금 이 순간**.
...
#### 토큰 갱신 — 지표를 6개월 뒤부터 갉아먹던 구멍 (2026-09-01)

활성 계측을 붙이고 나서 "로그인 토큰이 긴데 그럼 로그인 시점을 보는 거 아니냐"는 물음을 따라가다 찾았다.
답은 "아니다, 부팅 시점을 본다"였지만 **그 옆에 진짜 결함이 있었다.**

`signSession()` 호출부는 `auth/login`·`v1/devices` **둘뿐이고 갱신이 없었다.** 그래서 `iat + 180일`이
고정 카운트다운이 된다 — 앱을 매일 써도 그날이 오면 토큰이 죽는다. 그런데 bootstrap은 무효 토큰을
**401 없이 조용히 무시**하도록 설계돼 있어(진입 게이트가 세션 만료로 막히면 안 되므로), 그 사용자는:

- **DAU에서 영구히 사라진다** — 앱은 매일 쓰고 있는데
- 앱은 여전히 "로그인 돼 있다"고 믿는다 — `isSignedIn()`이 "저장소에 문자열이 있나"만 봤으므로
- 그걸 가드로 쓰는 앱의 `ensureDeviceSession()`이 **재등록을 영원히 건너뛴다**
- 앱에도 서버에도 신호가 없다 — 오류가 안 나고 로그에도 안 남는다

문의는 자가 치유된다(`/v1/tickets`가 401 → SDK가 세션 폐기 → 다음에 재등록). 하지만 **문의를 안 보내는
사용자는 영영 안 잡힌다** — 그게 절대다수다.

**고친 방법 — 두 겹**

1. **서버**: `bootstrap`이 유효 토큰을 받았고 발급 후 `TOKEN_RENEW_AFTER_MS`(30일 = TTL의 1/6)가
   지났으면 새 토큰을 발급해 `session.token`으로 응답에 싣는다. 앱을 6개월에 한 번만 열어도 갱신이 물리고,
   그러고도 정말 안 쓰는 기기는 결국 만료된다(영구 연장이 아니다). 재발급은 서명이라 DB를 안 건드린다.
   ⚠ **갱신할 때만** 주체 생존을 DB로 한 번 본다 — 탈퇴한 세션을 연장해주지 않기 위해서다.
   권한은 `requireSubject`가 따로 막지만, 죽은 세션을 연장하는 것 자체가 틀렸다.
2. **SDK**: `session.token`이 오면 조용히 교체한다(`replaceToken` — subject는 안 건드린다.
   사람이 바뀐 게 아니다). 그리고 `isSignedIn()`이 **만료를 판정**한다(`tokenAlive`) —
   1번으로도 못 살리는 "180일 넘게 안 켠" 경우의 안전망이다. 파싱 실패는 **죽은 것으로 본다**(fail-closed):
   깨진 토큰을 살아있다고 보면 위 공백이 그대로 재현되고, 죽은 것으로 보면 최악이 재발급 1회다
   (같은 deviceId면 같은 subject라 이력이 끊기지 않는다).

**타이밍**: 당시 가장 오래된 subject가 2026-08-06(약 1개월)이라 실피해는 0이었다. 2027-02 무렵부터
시작될 예정이었고, 증상이 "DAU가 왜 조금씩 줄지?"로만 나타나 사후 추적이 매우 어려웠을 것이다.
**출시 전에 잡은 것이 이 건의 전부다** — 나간 빌드는 수명 내내 그 버그를 안고 간다.

**앱 영향**: SDK를 `2026-09-01.2`로 **한 번 더 재복사**해야 한다. 호출부 변경은 없다.

#### 화면 규약 — 배구 콘솔 전수조사에서 가져온 것 (2026-09-01)

배구 운영 콘솔 19개 메뉴를 전부 열어봤다. 절반은 게임 전용(플레이·전지훈련·경기·선수·업적·광고·우편·쿠폰)이라
가져올 게 없고, **배울 것은 화면 구성이 아니라 "숫자를 어떻게 말하는가"였다.** 넷을 가져왔다.

1. **알림 0건에도 검사 항목을 나열한다** — 종전엔 "이상 징후 없음" 한 줄이라 정상인지 판정을 안 한
   것인지 구분이 안 됐다. 서버가 `alertChecks`를 내려보낸다(판정문 바로 옆에 둔다 — 딴 곳에 두면 반드시 어긋난다).
2. **출처 배지(`Src`) + 정의 캡션** — 배구는 지표마다 `자체-롤업`/`외부-sync`/`GA4 후` 배지를 달고,
   표 안에 정의와 **오독 금지 문구**를 쓴다("낮은 수치를 아무도 못 깼다로 읽지 마세요").
   우리도 대시보드 메타 줄(KST 경계·지표별 출처)과 활성 정의("하루에 여러 번 켜도 1명 · 기기를 바꾸면 다른 사람으로 잡힌다")를 박았다.
3. **미설정에 env 이름을 붙인다** — 배구는 "Sentry 조회 미설정 / SENTRY_API_TOKEN·SENTRY_ORG·SENTRY_PROJECT 필요"까지 쓴다.
   우리 배선 상태는 "미설정"까지만 말했다 — 그럼 거기서부터 env 이름을 찾는 게 일이 된다.
   ⚠ **이름만** 내려보낸다. 값이 섞이면 그 순간 콘솔로 웹훅 URL이 새는 것이라 가드가 URL 패턴을 막는다.
4. **신규 가입 차트 + 절대·상대 시각 병기** — 신규는 `subjects.createdAt` 파생이라 **계측과 무관하게 처음부터 쌓여 있다**
   — 활성이 0인 지금도 그려진다. 단 **두 계열을 한 차트에 섞지 않는다**(뜻이 다른 양을 같은 축에 놓으면 무의미한 비교를 유도한다).
   사용자 목록의 최근 접속은 `09-01 09:44 · 3시간 전`로 둘 다 — 상대만 있으면 로그 대조가 안 되고 절대만 있으면 암산을 해야 한다.

**안 가져온 것과 이유**: 내부 계정 제외(우린 subject에 그 축이 없다 — 가드 문의 격리가 같은 역할) ·
국가별 활성(country 미수집 — 수집하려면 처리방침을 고쳐야 하고 최소수집 원칙과 충돌) ·
우편·쿠폰·수동 지급·유저 원장(재화 경제가 없다) · 패치노트·개발자노트 분리(`announcements.kind`로 충분).

**미해결로 남긴 것**:
- ~~**시간대별 접속(0~23시)**~~ — **끝(2026-09-01)**. 두 단계로 풀었다.

  ① `lastSeenAt`을 하루 1회만 갱신하던 최적화를 되돌려 **최근 접속자(30분)** 타일을 붙였다.

  ② 분포 차트는 **`lastSeenAt`을 안 쓴다.** 그건 주체당 한 칸이라 그걸로 그리면 "몇 시에 사람이
  많나"가 아니라 **"몇 시에 마지막으로 껐나"** 가 된다(배구가 그 한계를 '마지막 접속 시각'이라고
  이름에 적어두고 쓰는 이유). 대신 `subject_active_day.hours`에 **KST 0~23시를 비트 24개로** 접어
  넣는다 — OR이라 멱등·순서무관이고, 시각 행을 따로 쌓을 때의 "하루 주체당 최대 24행"을
  **1행으로 고정**한다. 집계는 `generate_series(0,23)`으로 비트를 펼친다(24개 표현식을 손으로 쓰는
  것보다 틀릴 곳이 한 군데다).

  ⚠ **수집일 축이 둘이다**: 날짜는 8월부터, 시각은 2026-09-01부터. 이전 행은 `hours=0`이라
  히스토그램에 **한 건도 기여하지 않는다**(틀린 값 대신 없는 값). 화면도 그 축을 따로 보여주고,
  최소 3일치가 모이기 전에는 안 그린다 — 하루치를 패턴으로 읽으면 그날 우연히 몰린 시각이
  생활패턴으로 둔갑한다(요일 차트의 2주 게이트와 같은 이유).

  세는 단위는 **연인원(주체×날짜)** 이다. 같은 사람이 다른 날 같은 시각에 들어오면 2로 센다 —
  "그 시각에 사람이 있을 확률"을 보는 지표이지 순 사용자 수가 아니라고 화면에 적어뒀다.
- **리텐션 D1/D3/D7/D14** — 배구는 `근사 · lastSeenAt 기준` 배지를 달고 진짜 코호트와 뭐가 다른지를 적어둔다.
  우린 `subject_active_day`가 있어 **처음부터 진짜를 만들 수 있다** — 다만 D7은 7일치가 쌓여야 말할 수 있다.
- 실시간 접속(최근 30분) · 일/주/월 granularity 토글 · 점검 배너 다국어 · 결제 이벤트 필터 칩 · 차단된 위조 요청 카운트.

**우리가 이미 나은 곳**: 문의 화면(카테고리 필터·가드문의 격리+devCount 고지·CSV·모달 동시저장·앱 스코프 PATCH) ·
요일 차트 게이트(배구는 화면에서 `samples`로 판정하다 오판을 냈고, 우린 서버가 `chartReady`를 계산해 내려준다).

### Phase 13 — 웜 스타트 하트비트 + `exp` 클레임 (2026-09-02)

Phase 12가 붙인 계측에 **구멍이 하나** 있었다. 사용자가 찾았다 —
*"앱에서 홈 누르고 다시 들어가는 건 접속 정보로 안 나오는 것 같다."* 맞다.

#### 문제 — 하트비트가 프로세스당 1회였다

하트비트를 `fetchBootstrap()`에 얹어 둔 것까지는 맞았다(매 실행 불리는 유일한 라우트였으니까).
틀린 건 **"매 실행"을 "매 사용"으로 읽은 것**이다. 네 앱 전부 부팅 조회를 플래그로 묶어
**JS 프로세스당 1회**만 불렀고(`fetched`·`loaded`·`useEffect(…,[])`), RN에서 홈 버튼은 프로세스를
죽이지 않으므로 **포그라운드 복귀에는 하트비트가 한 번도 안 나간다.**

왜곡 셋이 전부 **방향이 나쁘다**:

| 지표 | 실제로 측정된 것 |
|---|---|
| DAU | 앱을 안 죽이는 사용자는 **다음 날부터 아예 안 잡힌다** — 열심히 쓸수록 덜 잡히는 거꾸로된 오차 |
| 최근 접속자(30분) | `lastSeenAt`이 콜드 스타트 시각에 고정돼 거의 항상 0 |
| 시간대별 활성 | "몇 시에 쓰고 있었나"가 아니라 **"몇 시에 처음 켰나"** 의 분포 |

#### 설계 — `POST /api/v1/heartbeat`

**새 라우트를 둔다. `bootstrap`을 다시 부르지 않는다** — 공지 50건 페이로드를 복귀마다 낚는 것도,
진입 게이트 판정을 다시 돌리는 것도 틀리다(점검 배너가 복귀할 때마다 다시 뜨게 된다).

**토큰에서 앱을 꺼낸다 — `?app=`을 받지 않는다.** `bootstrap`은 공개 라우트라 쿼리로 앱을 받지만,
여긴 **인증 라우트**다(`auth/me`·`tickets/mine`·`entitlements`와 같은 부류). 토큰에 서명된 `app`이
이미 진실이라 파라미터를 더하면 **어긋날 수 있는 자리를 만드는 것**뿐이고, 덤으로 앱 존재를
탐지할 면적도 생긴다. `requireSubject`가 토큰·주체의 `app`을 대조하고 탈퇴자를 걸러낸다.

**갱신은 여기가 더 좋은 자리다.** 슬라이딩 갱신(Phase 12)을 bootstrap에만 두면 **앱을 안 죽이는
사용자는 갱신도 안 받는다** — 같은 구멍이 둘을 동시에 만들고 있었다. 게다가 여긴 `requireSubject`가
주체를 이미 읽어오므로 **생존 확인이 공짜다**(bootstrap은 그걸 위해 쿼리를 하나 더 한다).

**401이다 — bootstrap과 반대다.** bootstrap은 무효 토큰을 조용히 무시하고 200을 준다(진입 게이트라
세션 만료가 점검 판정을 막으면 안 된다). 하트비트는 **감시할 게이트가 없고 관측이 유일한 임무**라,
토큰이 죽었다면 그걸 앱이 알아야 다음 부팅에 재등록한다(자가 치유). SDK가 401에 세션을 폐기한다.

#### 🔴 계약 — `heartbeat()`는 **절대 reject 하지 않는다**

이게 이 기능의 가장 중요한 설계 결정이다.

```ts
heartbeat(): Promise<void>   // 반환값 없음 · throw 없음
```

**왜 반환값을 안 주나**: `Result`를 돌려주면 호출부가 그걸 화면에 표시하고 싶어진다.
지금 안 그려도 다음 사람이 그린다. 포그라운드 복귀는 **부팅보다 훨씬 자주** 일어나므로,
그 순간 지하철에서 앱을 켜는 매번 오류가 뜨게 된다. **관측이 사용자 눈에 보이면 설계가 틀린 것이다.**

**⚠ 타입이 `void`인 것만으로는 부족하다** — 거부되는 프라미스는 그대로 샌다:
...
여기서 throw하면 **unhandled rejection**이다 — 개발 빌드는 빨간 박스, `ErrorBoundary`/`ErrorUtils`를
둔 앱은 **그게 그대로 오류 UI로 간다**. 그래서 네트워크 오류·타임아웃·미설정·**세션 없음**·
쿨다운 미충족·JSON 파싱 실패를 전부 내부에서 삼키고 조용히 resolve 한다.

🚫 **호출부에 `.catch(() => {})`를 요구하지 않는다.** 잊는 사람이 반드시 나오고, 잊었을 때의 증상이
정확히 이 API가 막으려던 그것이다. 방어를 밖으로 미루면 `void`를 고른 이유가 사라진다.
가드(`_dv_sdk.ts`)가 **서버가 죽어도 reject하지 않는지**를 직접 호출해 확인한다.
(my-word 세션이 짚었다 — 서명이 `void`인 것과 reject하지 않는 것을 내가 둘을 묶어 말했다.)

#### 쿨다운은 **SDK 안에** 둔다

5분. 앱마다 구현하면 어긋나고, 무엇보다 **쿨다운은 실패 처리와 같은 자리**라 한 곳에 있어야 한다.
서버는 이미 멱등(PK)이라 중복이 무해하지만, **네트워크를 아끼는 건 앱의 몫**이다.

⚠ 실패해도 쿨다운을 소모한다(시도 시각 기준). 오프라인일 때 복귀마다 재시도하면 또 폭주한다.
메모리에만 둔다 — 프로세스가 죽으면 다음은 콜드 스타트고, 그건 `bootstrap`이 이미 찍는다.

#### 덤 — 자정 넘김

밤 11시에 켜서 새벽 1시까지 쓰는 사용자가 종전엔 **11시 하루에만** 잡혔다.
이제 복귀할 때마다 찍히므로 두 날짜에 자연히 걸린다 — 따로 고칠 것이 없었다.

#### `exp` 클레임 — 만료 기준을 서버 한 곳으로

종전엔 만료 기준이 둘에 복제돼 있었다(서버 `TOKEN_TTL_MS` / SDK `SESSION_TTL_DAYS`).
값이 같아 무해했지만 **누가 서버만 바꾸면 조용히 갈라진다**(조각 세션 지적).

```
signSession    → { sid, app, iat, exp: iat + TOKEN_TTL_MS }
verifySession  → exp 있으면 exp 기준, 없으면 iat + TTL 폴백(기발급분 호환)
SDK tokenAlive → exp 우선, SESSION_TTL_DAYS 는 레거시 폴백으로 강등
```

그러면 서버가 유일한 진실이 되고, TTL을 바꿔도 다음 갱신(최대 30일)에 자동 전파된다 — **앱 재배포 불필요.**
**TTL 숫자는 180일 그대로 둔다** — `exp`가 들어가면 나중에 서버 한 줄로 바꿀 수 있으므로 지금 정할 필요가 없다.
(두 결정을 묶어놓고 계속 뒤집던 것을 linkmemo 세션이 분리해 풀었다 — `MULTI_SESSION.md §5 ⑥`.)

#### 배포 — 이번엔 AAB가 필요 없다

`AppState` 리스너 + SDK 교체라 **JS만 바뀐다.** `expo-updates`가 실린 빌드는 **OTA로** 받는다.
⚠ `runtimeVersion`은 **앱 버전이 아니다** — 네이티브 모듈이 바뀔 때만 올리는 게 그 앱들의 정책이라,
**앱 버전을 올려도 같은 OTA를 받는다.** 앱별 현황은 `C:\project\common\PLAY_CONSOLE_STATUS.md §2-4`.

**이번에는 앱 호출부가 처음으로 늘어난다.** 지금까지 SDK 갱신은 전부 재복사만으로 끝났지만,
여긴 `AppState` 리스너를 한 곳 붙여야 한다(cleanup 포함). 네 세션에 그걸 명시해 보낸다.
...
### Phase 14 — 정보 허브 (2026-09-07 결정 · 설계는 `docs/INFO_HUB.md`)

지원사업(기업마당·K-Startup 공식 API)·커뮤니티(스레드 키워드 검색)를 하루 1회 모아 콘솔에서 본다.
**여기엔 요약만 둔다** — 상세·근거·실패 모드는 전부 `docs/INFO_HUB.md`가 갖는다.

🔴 **이 저장소의 결정 셋을 뒤집는다. 그래서 여기 남긴다:**

| 기존 결정 | 정보 허브에서 | 왜 안전한가 |
|---|---|---|
| **관리자 write에 `appCode` 스코프 필수** | **없다** | 데이터에 앱이 없다. 오염될 남의 앱이 없다. 대신 *"이 화면은 앱과 무관"*을 화면에 박는다 |
| 공개 라우트는 무인증 write 방어가 필요 | **`/api/v1/*`를 안 만든다** | 독자가 운영자 한 명이다. 안 열면 레이트리밋·캡·CORS를 통째로 안 고민한다 |
| 앱 코드는 `apps` 테이블이 allowlist | **`info_sources` 표가 같은 역할** | 소스 추가에 재배포 불필요 — 같은 패턴을 그대로 쓴다 |

**Expand-only 유지**: 기존 9개 표를 하나도 안 건드리고 `info_sources`·`info_items` 둘만 더한다.
파기는 새 크론을 만들지 않고 기존 `cron/purge` + `lib/retention.ts`에 얹는다.

🚫 **제외 결정**(되살리기 쉬워 여기 적는다): HTML 크롤링 · 스레드 비공식 수집 · 실시간 수집 · 1차 AI 요약.
🔴 **사람 기준 스레드 수집은 불가능하다** — 메타 API에 엔드포인트 자체가 없다(2026-09-07 확인). 주제 기준으로 간다.
