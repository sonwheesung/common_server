...
**미해결로 남긴 것**:
- ~~**시간대별 접속(0~23시)**~~ — **막힘 해제(2026-09-01)**. `lastSeenAt`을 하루 1회만 갱신하던 최적화를
  되돌려 시간 해상도가 생겼다(단일 행 PK UPDATE라 아낀 값이 잃은 것보다 작았다). 그 위에 먼저
  **최근 접속자(30분)** 타일을 붙였고, 0~23시 분포 차트는 아직 안 만들었다 — 이제 만들 수 있다.
