...
**미해결로 남긴 것**:
- **시간대별 접속(0~23시)** — 공지 발행·점검 시각을 정할 때 쓰이는 지표인데 **지금 구조로는 못 만든다**.
  `subject_active_day`는 날짜만 들고, `lastSeenAt`은 부하를 줄이려고 **하루 1회만** 갱신하게 만들어 시간 해상도가 없다.
  넣으려면 그 최적화를 되돌리거나 시각 컬럼을 추가해야 한다(설계 시점에 이 지표를 몰랐다).
