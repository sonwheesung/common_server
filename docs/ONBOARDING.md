...
### 🔴 같은 날 `PRE_LAUNCH_CHECK.md §6`에 **행부터 만든다** (빈칸이어도)

`C:\project\common\PRE_LAUNCH_CHECK.md`의 프로젝트별 현황표가 프로덕션 출시 전 확인 목록이다.
**행이 없으면 그 앱은 점검 대상에서 통째로 빠진다** — 항목이 실패하는 게 아니라 **검사 자체가 없다.**

> 실측(2026-09-02): idea_repository가 프로덕션 액세스 승인을 받는 날까지 **그 표에 행이 없었다.**
> 그래서 아무도 안 본 채로 승인까지 갔고, 그제야 채우니 **오픈소스 고지가 0건**으로 드러났다
> (그 세션이 같은 날 신설했다 — 43개, 카피레프트 0).

⚠ **오픈소스 고지는 형제 앱이 전부 뒤늦게 닫은 항목이다**(배구·조각·linkmemo·my_word·농구·idea_repo).
매번 "출시 직전에 발견"으로 끝났다는 건 **점검 시점이 너무 늦다**는 뜻이다 —
등록 시점에 빈칸으로라도 행을 만들어 두면 그 칸이 계속 눈에 밟힌다.
빈칸(`—`)은 그 문서 규칙상 **"확인 안 함"이지 "없음"이 아니다.**

> 이건 오늘 가드에서 배운 것과 같은 병이다 — **없는 검사는 실패로 안 나타난다.**
> `MIN_CHECKS`가 "몇 개가 돌았나"를 세는 것처럼, 여기서는 **행이 있는가**가 그 역할을 한다.
...
🔴 **릴리스 번들에서 문구를 찾을 땐 — 반드시 있을 것을 같은 방법으로 먼저 센다.**
(조각 실측, 2026-09-02) `index.android.bundle`은 **Hermes 바이트코드**이고 비ASCII 문자열이
**UTF-16LE**로 들어간다. 그래서 같은 번들에서:

```
셸 grep '조각'            → 0    (셸이 한글 needle 을 못 넘긴다)
파이썬 UTF-8 바이트        → 0    (저장 인코딩이 다르다)
파이썬 \uXXXX 이스케이프  → 0    (바이트코드라 그 표기가 없다)
파이썬 UTF-16LE           → 70   ✅
```

⚠ **영어는 ASCII 라 첫 시도부터 잡힌다.** 그래서 화면상 *"영어만 실리고 한국어·일본어·중국어는
빠졌다"*로 보이고, 하마터면 **15개 언어 번역 누락으로 오진**할 뻔했다.

→ **0이 나오면 대상이 아니라 세는 방법을 먼저 의심한다.** 반드시 실려 있을 문자열(앱 이름 등)을
  같은 방법으로 세서 **0이 아닌지** 확인한 뒤에만 결과를 믿는다.
  이건 가드의 "대조군" 원칙과 같은 것이다 — **양쪽이 다 나와야 판정이 성립한다.**

🔴 **전송 빈도가 바뀌면 처리방침 문안도 본다** (2026-09-02).
웹 스타트 하트비트를 붙이면 **저장 항목은 그대로인데 전송 시점이 넓어진다**:

```
전:  앱 실행 시 1회
후:  앱 실행 시 + 포그라운드 복귀 시(최소 5분 간격)
```

⚠ **수집 항목이 안 늘었다고 문안이 그대로여도 되는 것이 아니다.**
Play 데이터 보안은 항목 기준이라 보통 그대로지만, **처리방침 본문은 따로 봐야 한다.**

🔴 **"1회"만 찾지 마라 — 빈도는 문장이 아니라 구조에도 숨는다.**
내가 처음 안내할 때 *"'1회'·'실행 시'류 서술이 있는지 보라"* 고 했는데 **너무 좁았다**(2026-09-02).
두 앱이 각기 다른 모양으로 걸렸다:

| 앱 | 걸린 자리 |
|---|---|
| `my_word` | 문장 4곳(국·영문 각 2) — *"앱을 실행할 때마다 … 조회 요청이 한 번 발생합니다"*. 집계 서술도 *여러 번 **실행**해도* → *여러 번 **보내더라도*** 로 고쳤다(실행만이 계기가 아니니까) |
| `jogak` | **표제** — *"바. 앱을 실행하시는 경우"*. §2가 수집 항목을 **수집 시점별로 묶은 구조**라 그 표제가 곷 빈도 서술이었다 |

→ 문장만 생각하지 말고 **문서가 시점으로 구조화돼 있는지**를 먼저 본다.

🔴 **세션을 나눠 쓰는 앱은 한 겹 더 있다.**
로그인 사용자의 활성 일자가 **기기 식별자가 아니라 계정으로** 기록된다면, 문안의 계정 항목
이용 목적에 그게 적혀 있어야 한다. 조각은 가목(계정) 목적이 *"문의자 확인·답변 회신"* 뿐이라
**가목만 읽는 사람은 자기 계정이 통계에 쓰이는 걸 모른다**— vc16부터 이미 그러했다(2026-09-02 발견).
→ *"로그인하신 경우에는 기기 식별자 대신 로그인 계정으로 기록되며, **한 분이 두 번 집계되지
않도록** 하기 위함입니다"* 처럼, 설계 결정을 **이용자 언어로** 옮긴다.

⚠ **다국어 처리방침은 항목 기호가 언어마다 다르다** (jogak 실측, 14개 언어):

```
바. (ko)   カ． (ja)   己. (zh-Hans/Hant)   е. (ru)   ฉ. (th)   f. (나머지 9개)
```

치환 스크립트를 `'f.'`로 찾게 지으면 ja에서 멈추고 **ko만 적용된 상태**로 남는다 —
언어마다 내용이 달라지는 가장 나쁘 종류의 실패다. **기호가 아니라 모양**(표제 + 불릿 + 근거 + 목적)으로
자리를 잡고, 적용 후 **언어 수만큼 바뀌었는지**를 센다.

⏱ **순서는 한 방향뿐이다: 문안 게시 → 새 동작 도달.**
그 사이에는 **방침이 앱보다 넓게** 적혀 있는데 그건 무해하다 — **좁게 적힌 것이 위반**이지
넓게 적힌 건 아니다. 반대로 하면 그 기간이 그대로 불일치 창이 된다.
⚠ OTA 는 **발행이 곷 도달**에 가깝다(다음 실행부터 적용) — "나중에 쓰면 된다"로 미루면 창이 생긴다.

⏱ **이미 게시된 문안을 고치는 앱은 시행일·고지 기간을 본다.**
미게시 rev 에 얹으면 **불일치 창이 생기지 않는다** — idea_repository 가 rev.6(미게시)에 편입한 방식이 그것이다.
게시 뒤에 바꾸면 그 사이가 "문안과 실제가 다른 기간"이 된다.
...
**Play Console 데이터 보안 — 대개 "항목 추가"가 아니라 "목적/이유 칸 채우기"다**

광고를 붙인 앱은 AdMob GAID 때문에 `기기 또는 기타 ID`가 **이미 선언돼 있다.** 그래서 3단계(데이터 유형)는
손댈 게 없고, 실제로 손대는 건 4단계(데이터 취급 및 처리)다. **다만 무엇이 비어 있는지는 앱마다 다르니
기존 선언 상태를 먼저 열어보고 판단할 것** — 2026-09-01 같은 날 두 앱이 서로 다른 칸을 채웠다:

| 앱 | 기존 선언 | 실제로 바꾼 것 |
|---|---|---|
| `my_word` | 광고 또는 마케팅 · 사기 예방·보안·규정 준수 | 수집 목적에 **앱 기능** 추가 |
| `linkmemo` | 수집됨·공유됨 · 데이터 수집 필수 | 비어 있던 수집 이유 **애널리틱스** 체크 |

- `앱 활동 / 기타 사용자 제작 콘텐츠`(문의 본문)가 이미 "수집됨·선택적·앱 기능"이면 변경 없다
- 2단계 계정 문항은 **유지**한다 — 우리 UUID는 계정이 아니다("계정을 만들도록 허용하지 않음")
- ⚠ 우리 식별자는 **광고 식별자와 별개**다. 처리방침에서도 기존 GAID 조항과 섞지 말고 따로 기술한다

⏱ **제출 타이밍은 둘 중 하나로 갈린다.** 앱·처리방침·데이터 보안을 한 건으로 묶어 심사자가 같은 시점에
보게 하거나(my_word), 이미 심사가 돌고 있으면 **게시 완료를 기다렸다가** 데이터 보안을 고친다(linkmemo) —
심사 중에 고치면 진행 중 심사와 얽힌다.
