...
| 잘 쓰던 사용자가 어느 날부터 DAU에서 빠짐 | 세션 토큰 만료(발급 후 180일). SDK가 `2026-09-01.2` 미만이면 갱신을 못 받는다 | SDK 버전. 갱신은 발급 30일 후부터 `bootstrap` 응답의 `session.token`으로 온다 |
| 앱은 로그인 상태인데 문의·구독이 전부 401 | 위와 같은 만료. `2026-09-01.2`부터 `isSignedIn()`이 만료를 보고 false를 준다 | SDK 버전 |
