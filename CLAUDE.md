...
🔴 **쓸 때는**: 이스케이프가 든 내용은 heredoc 말고 **Write·Edit 도구로** 쓴다. 굳이 스크립트라면 `chr(92)` 로 조립한다.
...
⚠ **그리고 파이썬 `open(...,'w')` 는 인코딩 예외가 나기 전에 파일을 먼저 비운다** —
2026-09-08 에 이 파일이 실제로 **0바이트가 됐다**(surrogate 이스케이프 하나 때문에). `git checkout` 으로 복구했고
그날 커밋 안 된 편집은 잃었다. **커밋 안 된 파일을 스크립트로 덮어쓰지 않는다.**
