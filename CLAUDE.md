...
- **커밋 규약**: `C:\project\common\COMMIT_CONVENTION.md` (형제 앱과 공유하는 정본)
  🔴 **값을 여기 베껴 적지 않는다.** 2026-09-09 대조에서 여덟 프로젝트 중 **다섯이 베껴 적고 있었다.**
  그건 그 문서 서문이 "이게 문제였다"고 지목한 바로 그 형태다. 원본이 바뀌어도 안 따라온다.
- **한국어 문장 규칙**: `C:\project\common\KOREAN_WRITING.md` (형제 앱과 공유하는 정본)
  문장 안 줄표 금지, 사용자용 한국어는 검수. 🔴 **여기에도 값을 베껴 적지 않는다.**
...
🔴 **가드가 죽는 원인에는 *진화형* 이 따로 있다 — 제품이 바뀌었는데 그걸 지켜보는 검사를 같이 안 고친 것**
(2026-09-09 배구명가 세션이 하루에 여덟 건 찾아 §4.1 가드 결함 레지스트리로 등록했다).
아래 "이스케이프가 정규식을 먹는다"는 **기계적** 원인이고, 이건 **의도한 변경의 부수효과**라 성격이 다르다.
코드는 멀쩡하고 검사도 멀쩡한데 **둘의 전제가 어긋난다**:

```
표시 문자열을 t() 로 감쌌다        → 화면 소스를 정규식으로 읽는 검사가 눈먼다
기능 플래그를 껐다                 → 그 기능의 표본을 요구하는 검사가 **구조적으로 통과 불가**가 된다
상수를 다차원화했다(포지션별·리그별) → 그 값을 리터럴로 복제한 검사가 거짓 경보를 낸다
엔진이 바뀌어 효과 크기가 줄었다    → z 문턱은 N 의존이라 회귀가 아닌데 상시 빨간불이 된다
```

⚠ **이 저장소에 대입해 봤다**(2026-09-09): 커뮤니티 수집을 껐으므로 두 번째가 걸릴 자리였는데,
`_dv_info` 의 community 검사는 **상태코드만 본다**(건수를 요구하지 않는다) — 해당 없었다.
세 번째도 확인했다: 가드가 `SUMMARY_MAX` 를 `lib/info.ts` 에서 **import** 한다(복제 아님).

🔴 **대신 다른 게 나왔다 — 바닥이 밀려 있었다.**
`_dv_info` 가 55/67 개를 도는데 `MIN_CHECKS` 는 52/63 이었다. **넷이 죽어도 안 걸리는 상태**였다.
다른 세 가드는 실행수와 바닥이 정확히 같았고 이것만 어긋나 있었다 — **검사를 늘리면서 숫자를 안 올린 것**이다.
→ **바닥은 실행수와 같아야 문다.** 검사 하나를 지워 변이로 확인했다(`FAILED — 실행 67 / 최소 67`).
...
🔴 **문서만의 문제도 아니다. 소스 코드에도 박힌다**(2026-09-09 Re:Read 실측).
`features/backup/merge.ts` 의 열쇠 구분자가 **NUL(0x00)** 로 박혀 있었다.

```
tsc  ·  lint  ·  기능 가드    전부 초록
바이트 판독기                  🔴 0x00@13
```

**동작이 안 바뀌므로 동작을 재는 검사로는 영원히 안 잡힌다.** 직접 재현해 확인했다.
`tsc --noEmit` 에 NUL 이 든 파일을 물려도 **아무 말도 하지 않는다.**
⚠ 그래서 검사 대상에 `.ts`·`.tsx` 를 반드시 넣는다. 이 저장소는 git 추적 77개를 80개로 덮고 있다(확인).
...
| `linkmemo` | `C:\project\link_memo` (Expo, 로그인 없음) | **2026-09-02** | 🟢 프로덕션 **12/1.0.8**(R8 도입 · 177개국 · **2026-09-09 12:24 게시**). ~~11/1.0.7 · 2026-09-07 실측~~. 웜 스타트 하트비트 부착(OTA). `ensureDeviceSession()` 기준 구현체.<br>⚠ **출처는 LinkMemo 세션의 콘솔 실측 보고**다 — 내가 직접 본 값이 아니다. 신규 설치 27.0 → **20.5MB** |
